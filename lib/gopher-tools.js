...
const GopherTime = require('./gopher-time')


class GopherTools extends GopherTime
{
  constructor() {
    super()
  }
  ...
  *******************************************************************/
  stringStartsWithSelect (sqlStatement) {
    let regexExpression = /^select/ig,
        beginsWithSelect = sqlStatement.match(regexExpression);
    if (beginsWithSelect) {
      super.gopherChatter(
        'debug',
        'string starts with select: true')
      return true }
    super.gopherChatter(
      'debug',
      'string starts with select: false')
    return false
  }
  ...
  *******************************************************************/
  setConnectionString (connectionConfig) {
    return new Promise((resolve, reject) => {

      //some validation of connection Object: host port and service or SID must be present
      if (connectionConfig.host && connectionConfig.port && connectionConfig.service ||
          connectionConfig.host && connectionConfig.port && connectionConfig.SID){

        //host and service (or SID) must be a string, and port must be a number
        if(connectionConfig.service.constructor, connectionConfig.host.constructor === String && connectionConfig.port.constructor === Number||
           connectionConfig.SID.constructor, connectionConfig.host.constructor === String && connectionConfig.port.constructor === Number){

          //append new connectionString to connectionConfig
          for(var key in connectionConfig){
            if(connectionConfig.hasOwnProperty(key)){
              if(key==="service"){
                Object.assign(
                  connectionConfig,
                  { "connectString" : connectionConfig.host+':'+ connectionConfig.port+'/'+connectionConfig.service } )
                super.gopherChatter(
                  'debug',
                  'connectString: '+ connectionConfig.host+':'+ connectionConfig.port+'/'+connectionConfig.service )
                resolve(
              ...
              if(key==="SID"){
                Object.assign(
                  connectionConfig,
                  { "connectString" : connectionConfig.host+':'+ connectionConfig.port+'/'+connectionConfig.SID } )
                super.gopherChatter(
                  'debug',
                  'connectString: '+ connectionConfig.host+':'+ connectionConfig.port+'/'+connectionConfig.SID )
                resolve(
          ...
          let gophErrMsg = 'Failed to build connection string. Check Host, Port, and either Service or SID formats. Port must be a number, and Host and Service or SID must be a string.'
          super.gopherChatter(
            'gophErr',
            gophErrMsg )
          reject(
        ...
        let gophErrMsg = 'Each Connection must have it\'s own defined values for Host, Port, and either Service or SID'
        super.gopherChatter(
          'gophErr',
          gophErrMsg )
        reject(
  ...
  /******************************************************************
  Set defaults or apply input overides to transactionConfig dataOnly
  *******************************************************************/
  setTransactionConfiguration (transactionInputs, transactionDefaults, transactionConfig) {
    let transConfig = transactionConfig
    //-------------Bind Variables
      if (!transactionInputs.bindVariables) {
        if (!transactionConfig["bindVariables"]) {
          Object.assign(
            transConfig,
            { "bindVariables": {} } ) } }
      else{Object .assign(
        transactionConfig,
        { "bindVariables" : transactionInputs.bindVariables } ) }
    //-------------Output Format
      if (!transactionInputs.outputFormat) {
        if (!transactionConfig["outputFormat"]) {
          Object.assign(
            transConfig,
            { "outputFormat" : transactionDefaults.outputFormat } ) } }
      else{Object.assign(
        transactionConfig,
        { "outputFormat" : transactionInputs.outputFormat } ) }
    //-------------Zero Row Message
      if (!transactionInputs.zeroRowMessage) {
        if (!transactionConfig["zeroRowMessage"]) {
          Object.assign(
            transConfig,
            { "zeroRowMessage" : transactionDefaults.zeroRowMessage } ) } }
      else{Object.assign(
        transactionConfig,
        { "zeroRowMessage" : transactionInputs.zeroRowMessage } ) }
    //-------------Time Out
      if (!transactionInputs.timeOut) {
        if (!transactionConfig["timeOut"]) {
          Object.assign(
            transConfig,
            { "timeOut" : transactionDefaults.timeOut } ) } }
      else{Object.assign(
        transactionConfig,
        { "timeOut" : transactionInputs.timeOut } ) }
    //-------------Time Out Message
      if (!transactionInputs.timeOutMessage) {
        if (!transactionConfig["timeOutMessage"]) {
          Object.assign(
            transConfig,
            { "timeOutMessage" : transactionDefaults.timeOutMessage } ) } }
      else{Object.assign(
        transactionConfig,
        { "timeOutMessage" : transactionInputs.timeOutMessage } ) }
    //-------------Row Limit
      if (!transactionInputs.maxRowsReturned) {
        if (!transactionConfig["maxRowsReturned"]) {
          Object.assign(
            transConfig,
            { "maxRowsReturned" : transactionDefaults.maxRowsReturned } ) } }
      else{Object.assign(
        transactionConfig,
        { "maxRowsReturned" : transactionInputs.maxRowsReturned } ) }
    //-------------Response Output
      if (!transactionInputs.responseOutput) {
        if (!transactionConfig["responseOutput"]) {
          Object.assign(
            transConfig,
            { "responseOutput" : transactionDefaults.responseOutput } ) } }
      else{Object.assign(
        transactionConfig,
        { "responseOutput" : transactionInputs.responseOutput } ) }
    //-------------Time Zone
      if (!transactionInputs.timeZone) {
        if (!transactionConfig["timeZone"]) {
          Object.assign(
            transConfig,
            { "timeZone" : transactionDefaults.timeZone } ) } }
      else{Object.assign(
        transConfig,
        { "timeZone" : transactionInputs.timeZone } ) }
    super.gopherChatter(
      'debug',
      'transConfig: '+ JSON.stringify(transConfig) )
    return transConfig
  }
  ...
  Statment
  *******************************************************************/
  fetchBindVariableNames (dbStatement) {
    return new Promise((resolve, reject) => {

      //err if dbStatement is not a string
      if (dbStatement.constructor != String) {
        let gophErrMsg = 'Db Statement must be a string'
        super.gopherChatter(
          'gophErr',
          gophErrMsg )
        reject(
  ...
  given name
  *******************************************************************/
  fetchInputs (name,libraries) {
    return new Promise((resolve, reject) => {
      if (name === undefined || libraries === undefined){ //if name or libraries are undefined
        let gophErrMsg = 'Inputs for both \"Name\" and \"Libraries\" must not be undefined'
        super.gopherChatter(
          'gophErr',
          gophErrMsg )
        reject(
          gophErrMsg ) }
      if(name.length === 0 || libraries.length === 0){//if name or libraries are blank
        let gophErrMsg = 'Inputs for both \"Name\" and \"Libraries\" must not be blank'
        super.gopherChatter(
          'gophErr',
          gophErrMsg )
        reject(
          gophErrMsg ) }
      if (name.constructor  !== String) {//if name is not a string
        let gophErrMsg = 'The \"Name\" input must be a String'
        super.gopherChatter(
          'gophErr',
          gophErrMsg )
        reject(
          gophErrMsg ) }
      if (libraries.constructor !== Array) {//if libraries is not an array
        let gophErrMsg = 'The \"Libraries\" input must be an Array'
        super.gopherChatter(
          'gophErr',
          gophErrMsg )
        reject(
      ...
      let response
      for (let i = 0; i < libraries.length; i++){//loop through json text file line by line
        try {// if file not found: handle; if file found: continue on
          if (fs.statSync(libraries[i])) {/*Do Nothing*/} }
        catch(e) {
          let gophErrMsg = 'The following file was not found: '+libraries[i]
          super.gopherChatter(
            'gophErr',
            gophErrMsg )
          reject(
       ...
       //If string var is JSON....
        if (this.isJson(string)===true) {
         ...
         //Search for name
          ...
          let gophErrMsg = 'Library files must be in JSON format'
          super.gopherChatter(
            'gophErr',
            gophErrMsg )
          reject(
      ...
      super.gopherChatter(
        'debug',
        'fetchedInputs: '+ JSON.stringify(response) )
      resolve(response) })
  }
  ...
  *******************************************************************/
  sqlQueryBuilder (sqlStatement,swapValues) {
    return new Promise((resolve, reject) => {
      let regexExpression = /\B<[\w]+\b\}~\B/g, //find all words (or any variation of contiguous letters and numbers) that are bound between a "LESS THAN" and a "CLOSE CURLY BRACE TILDE" e.g. <Some Variable Here}~
          bindVariableExtract = sqlStatement.match(regexExpression), //every bind variable in the sqlStatement
          bindVariableArray = [],
          sqlstr = sqlStatement;
      ...
      for (let i = 0; i < bindVariableExtract.length; i++) {
        let isDistinct = bindVariableArray.indexOf(bindVariableExtract[i])
        if (isDistinct === -1) {
          bindVariableArray.push(bindVariableExtract[i])}
      }

      //some error checking...
      let noOfBindVariables = bindVariableArray.length,
          noOfValues = swapValues.length;

      if (noOfBindVariables !== noOfValues) {
        let gophErrMsg = 'SQL Statement Rejected: The number of \"Swap Values\" entered does not match the number of \"Swap Variables\" types found in the SQL Statement'
        super.gopherChatter(
          'gophErr',
          gophErrMsg )
        reject(
        ...
        for (let i = 0; i < bindVariableArray.length; ++i) {
          let stillExists = sqlstr.indexOf(bindVariableArray[i])

          while (stillExists !== -1){
            sqlstr = sqlstr.replace(bindVariableArray[i],swapValues[i])
            stillExists = sqlstr.indexOf(bindVariableArray[i]) }}

        if (this.stringStartsWithSelect(sqlstr) === false) {
          let gophErrMsg = 'SQL Statement Rejected: SQL Statments must begin with the word \"SELECT\"'
          super.gopherChatter(
            'gophErr',
            gophErrMsg )
          reject(
            gophErrMsg ) }
        if (this.rejectInject(sqlstr) !== false) {
          let gophErrMsg = 'SQL Statement Rejected: The following SQL phrases are not allowed: '+ this.rejectInject(sqlstr)
          super.gopherChatter(
            'gophErr',
            gophErrMsg )
          reject(
...
//   //-------------Swap Values
//     if (!transactionObjOverides.swapValues) {
//       if (!transactionObject["swapValues"]) {
//         Object.assign(transactionObject,{"swapValues":{}});
//       }
//     }else{Object.assign(transactionObject,{"swapValues":transactionObjOverides.swapValues});}
//   //-------------Output Format
...
module.exports = //{
  GopherTools //: GopherTools,
  // stringStartsWithSelect: stringStartsWithSelect,
  // rejectInject : rejectInject,
  // sqlQueryBuilder : sqlQueryBuilder,
  // isJson : isJson,
  // fetchInputs : fetchInputs,
  // fetchBindVariableNames : fetchBindVariableNames,
  // setProperties : setProperties,
  // setConnectionString : setConnectionString,
  // fetchConnectionInputs : fetchConnectionInputs
